...
- network inspection, live element-tree inspection, React Navigation state inspection, Tanstack Query 관찰을 위한 focused plugin
...
- `@react-native-scalable-devtools/react-navigation-plugin`: React Navigation plugin입니다. 외부 LLM agent가 `navigationRef`를 등록하고 debugger frontend에서 navigation state를 실시간으로 확인해야 할 때 사용합니다. [패키지 README](packages/react-navigation-plugin/README.ko.md) 참고.
