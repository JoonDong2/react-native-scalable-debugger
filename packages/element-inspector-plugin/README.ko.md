...
`react-native-scalable-debugger`를 위한 live React Native element tree inspector plugin입니다.
...
`compact=1&plain=1`을 함께 사용하면 tree를 먼저 compact 처리한 뒤 plain text로 렌더링합니다.
...
network plugin 없이도 사용할 수 있지만, 두 plugin은 모두 `react-native-scalable-debugger`의 동일한 core AppProxy 모델을 공유합니다.
