...
첫 요청은 이후 요청보다 눈에 띄게 느릴 수 있습니다. 서버가 client-side plugin code를 주입하고, 앱 runtime 연결을 기다리고, 첫 snapshot을 반환하기 전에 React tree를 측정해야 할 수 있습니다. agent나 test timeout을 설정할 때 이 점을 고려하세요.
...
- `GET /react-navigation/state`는 앱 runtime에 등록된 React Navigation state를 요청합니다.
- `POST /react-navigation/navigate`와 `POST /react-navigation/back`은 앱 runtime에 semantic React Navigation action 수행을 요청합니다.
